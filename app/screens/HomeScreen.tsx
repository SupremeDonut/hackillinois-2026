...
const VOICES = {
    ...
    Brock: {
        description: "A commanding and loud drill sergeant",
...
export default function HomeScreen() {
    ...
    // Initialize with the first voice from the VOICES object
    const [voice, setVoice] = useState<VoiceKey>(
        Object.keys(VOICES)[0] as VoiceKey,
    );
    ...
    const handleAddGoal = async () => {
        ...
        if (!activityType.trim()) {
            Alert.alert(
                "Select activity",
                "Please select an activity first so the goal can be tracked correctly.",
            );
            return;
        }
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: Colors.background }}>
            ...
            >
                ...
                <View style={S.brand}>
                    ...
                    <TouchableOpacity
                        ...
                        onPress={() =>
                            ...
                            navigation.navigate("Recording", {
                                ...
                                description:
                                    description.trim() ||
                                    `I want to improve my ${activityType} form.`,
                    ...
                    >
                        <Text
                            style={[S.ctaText, !isReady && S.ctaTextDisabled]}
                        >
                            Start Recording
                        </Text>
                    </TouchableOpacity>
                    ...
                    {addingGoal && (
                        <View style={S.card}>
                            ...
                            <TextInput
                                ...
                                placeholder={
                                    activityType
                                        ? `e.g. Nail my ${activityType} follow-through`
                            ...
                            <TouchableOpacity
                                style={[
                                    S.cta,
                                    !newGoalText.trim() && S.ctaDisabled,
                                ]}
                    ...
                    {goals.length === 0 && !addingGoal && (
                        <Text style={S.goalsEmpty}>
                            No goals yet. Add one to track your progress over
                            time.
                        </Text>
                    )}
                    ...
                    {goals.map(goal => {
                        ...
                        const positive =
                            (latestRun?.improvement_delta ?? 0) > 0;
                        return (
                            <TouchableOpacity
                                key={goal.id}
                                style={S.goalCard}
                            ...
                            >
                                <View style={S.goalCardLeft}>
                                    <Text style={S.goalCardActivity}>
                                        {goal.activityType}
                                    </Text>
                                    <Text
                                        style={S.goalCardName}
                                        numberOfLines={2}
                                    >
                                        {goal.name}
                                    </Text>
                                    <Text style={S.goalCardMeta}>
                                        {goal.runs.length} session
                                        {goal.runs.length !== 1 ? "s" : ""}
                                        {latestRun
                                            ? ` · Last score: ${latestRun.score}`
                                            : ""}
                                    </Text>
                                </View>
                                <View style={S.goalCardRight}>
                                    {latestRun ? (
                                        <Text style={S.goalCardScore}>
                                            {latestRun.score}
                                        </Text>
                                    ) : (
